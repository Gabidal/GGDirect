...
# ----------------------------------------------------------------------------
# This script automates profiling for the GGDirect project using Valgrind (with 
...
show_help() {
    echo "Usage: $0 [OPTION]"
    echo "Profiling script for the GGDirect project with Valgrind and Callgrind."
    echo
    echo "Options:"
...
echo "Running GGDirect with Valgrind and Callgrind enabled..."
...
valgrind $CURRENT_FLAG --callgrind-out-file=callgrind.out "$current_dir/bin/build/GGDirect" || handle_error "Valgrind profiling failed."
